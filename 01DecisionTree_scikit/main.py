...
if not ( os.path.isfile(Pickle_Dir + "DecisionTree_classifier_scikit.pickle") \
            and os.path.isfile(Pickle_Dir + "DecisionTree_MultiLabelBinatizer_scikit.pickle") ):
    # Decision Tree Classification
    clf = tree.DecisionTreeClassifier()
    clf = tree.DecisionTreeClassifier(min_samples_leaf=3, min_weight_fraction_leaf=0.2, max_depth=500)
    ...
    with open( Pickle_Dir +  "DecisionTree_classifier_scikit.pickle" , "w" ) as fh:
        cPickle.dump( clf, fh)
    with open( Pickle_Dir +  "DecisionTree_MultiLabelBinatizer_scikit.pickle" , "w" ) as fh:
        cPickle.dump( mlb, fh)

clf = cPickle.load( open( Pickle_Dir +  "DecisionTree_classifier_scikit.pickle" , "r" ) )
mlb = cPickle.load( open( Pickle_Dir +  "DecisionTree_MultiLabelBinatizer_scikit.pickle" , "r" ) )
...
# main()  # [Finished in 537.8s], [Parser] Total number of passages parsed: 10997.
#[Accuracy] Total: 2211, Correct: 1800, Percentage: 0.81
